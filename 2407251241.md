...
There are various methods for authenticating on an SSH server, with the default being password authentication. However, I have determined that this default method is not the most secure approach. In terms of both convenience and security, SSH key-based authentication, from my research, appears to be a better option in both regards.[^1]
...
## creating SSH keys
First, an SSH key needs to be generated on the client device using `ssh-keygen`, which is included in the `openssh` package. By default, OpenSSH versions 9.5 and above will generate an ed25519 key pair.[^2]
